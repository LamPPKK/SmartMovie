...
| Movie and TV detail | `/v2/titles/{mediaType}/{id}` | UI | Apple, native Android, and KMP expose tagline, creators/credits, collection, navigable companies/networks, seasons, external IDs, images, videos, reviews, recommendations, similar titles, providers, and a typed release/localization section. Regional certification/date selection, alternative-title ordering, translations, missing nullable values, and unknown fields share canonical conformance fixtures. |
...
| TV seasons and episodes | `/v2/tv/{seriesId}/seasons/{seasonNumber}` and episode child route | UI | Season/episode metadata, credits, guest stars, images, videos, and air dates are represented. Safe episode context mirrors to Apple Watch/Wear OS and opens the exact series/season/episode back on the paired phone without enabling title-only trailer or library mutations. |
