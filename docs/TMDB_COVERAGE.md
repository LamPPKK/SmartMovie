...
| Movie and TV detail | `/v2/titles/{mediaType}/{id}` | UI | Apple, native Android, and KMP expose tagline, creators/credits, collection, navigable companies/networks, seasons, external IDs, reviews, recommendations, similar titles, providers, and a typed release/localization section. Non-empty canonical media fixtures drive deduplicated image galleries and every valid YouTube video; blank paths/keys, duplicate assets, and non-YouTube videos stay hidden. Regional certification/date selection, alternative-title ordering, translations, missing nullable values, and unknown fields share canonical conformance fixtures. |
