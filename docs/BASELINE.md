...
## Apple watched-episode progress — 29 August 2026

- Episode Detail can mark one episode watched/unwatched. Season Detail shows a completion value/bar, exposes a full-width control for every episode, and can mark all listed episodes watched/unwatched. Progress uses the canonical `series:season:episode` identity and never enters the Worker/TMDb account outbox.
- SwiftData adds `EpisodeWatchRecord` to the existing schema. An on-disk regression creates the previous two-entity library, writes a Favorite+Watchlist row, opens it with the additive progress schema, writes progress, destroys/reopens the container and verifies all three states remain intact. Individual, bulk, idempotency, isolation and duplicate-reconciliation tests also pass.
- Independent review required a compact-layout and VoiceOver correction before approval. The final row places the full-width episode card above a 44-point labeled action; rendered evidence covers a 272-point viewport at Accessibility 5, and all six shipped action/episode-label pairs include episode number and title.
- Final verification passed 100/100 Swift package tests and strict SwiftLint across 83 files. Unsigned iOS Simulator and tvOS Simulator builds passed after the layout correction.
- Native Android/KMP parity, actual VoiceOver/D-pad gestures, live CloudKit multi-device refresh and production CloudKit schema deployment remain open. This Apple slice does not claim full PAR-PROGRESS-001 completion.
