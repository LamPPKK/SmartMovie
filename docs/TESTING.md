...
| Adult content gate | 2 | Explicit age confirmation, matching six-digit PIN validation, five-attempt lockout, and five-minute recovery |
| Adult catalog visibility | 2 | Title, nested Person-known-for, credit, and fail-closed Season/Episode filtering for entity-related surfaces with additive missing/false/true adult metadata compatibility |
| Watch remote | 3 | Remote presentation intent, dismissal, Library revision, title context, and exact episode context |
| Catalog media presentation | 3 | Stable image/video deduplication, playable YouTube filtering, URL construction, and non-blank sorted external identifiers |
| Catalog editorial presentation | 2 | Non-blank review filtering, stable review-ID deduplication and four-item cap, localized author fallback inputs, recommendation deduplication, current-title exclusion, and fail-closed adult-title filtering |
| Cloudflare Worker | 98 | `/v1` + `/v2` schema/fixture validation, Movie/TV alternative-title envelope mapping, numeric and string Season/Episode external-ID normalization, configured capability/fixture equality, malformed broker configuration and mis-scoped return-URI allowlist rejection, advanced Movie/TV Discover forwarding and type-specific rejection, real calendar-date validation, normalized regional provider configuration, adult-partitioned External ID/entity/Credit Detail normalization, account-recommendation v4 mapping, normalized/paginated mixed lists, canonical list-ID replay, PUT/PATCH update compatibility, private `no-store`, cache/rate limits, TMDb Changes pagination/14-day backlog recovery, invalid cursor and verified changing-page-count recovery, UTC-date replay idempotency, D1-safe 100-item page chunking, monotonic entity revision invalidation, D1-read cache bypass, repeatable D1 migrations, Wrangler scheduled-bundle validation, encryption, callback/CSRF/CORS controls, sessions, and idempotent mutation replay |
| Protected account smoke runner | 9 | Full Movie/TV Favorite and Watchlist matrix, Movie/TV/Episode rating restoration, account recommendations, mixed-list create/update/add/comment acknowledgement/remove/verified deletion, stable 429/5xx/`mutation_in_progress` retry idempotency, response-body loss, success/error privacy and Worker-version invariants, missing-state rejection, and orphan cleanup after failure |

Current verified baseline: 67 Swift tests, 98 Worker unit/contract tests, and 9 protected-account smoke-runner tests. Historical evidence and the local toolchain boundary are recorded in [Baseline](BASELINE.md).

## Latest local verification

The 28 August 2026 implementation run passed all 67 SmartMovieKit tests, all 98 Worker unit/contract tests, all 9 protected-account smoke-runner tests, and Worker type-checking. The account runner now snapshots and restores all four Movie/TV Favorite/Watchlist states plus Movie/TV/Episode ratings, validates both recommendation feeds, exercises mixed-list metadata and item mutations through verified deletion, keeps one idempotency key across transient retry and lost response bodies, rejects non-private or mixed-version success/error responses, and discovers cleanup candidates by their unique CI name after a mid-flow failure. Contract coverage also verifies canonical list IDs on first and replayed creation, Worker `include_adult` partitioning and defensive client filtering for External ID, Person known-for/credits, Collection parts, Company/Network/Keyword titles and Credit Detail, plus non-empty catalog reviews, same-media-type TMDb recommendations, and separate similar titles; deterministic blank filtering, stable review/recommendation deduplication, four-review presentation limits, current-title exclusion, and fail-closed adult filtering for both related-title shelves; non-empty Movie/TV/Season/Episode image and YouTube-video fixtures; stable media deduplication/filtering; numeric and string Season/Episode external-ID normalization; Season/Episode runtime/production/vote metadata and external IDs; typed Movie/TV alternative titles; Movie release events; TV content ratings; translations; missing nullable values; unknown fields; exact-region release selection; and localized title preference. Companion coverage verifies safe title context, exact series/season/episode handoff, backward-compatible paired-device messages, and suppression of episode trailer/library actions. Account rollout coverage verifies canonical `browser_auth`/`tv_qr_auth` keys, configured Worker response/fixture equality, rejection of short encryption keys, malformed/insecure callback origins, and empty or mis-scoped return-URI allowlists, missing/false/true fail-closed behavior, cold-start callback deferral, stale async-completion invalidation, durable outbox isolation, and localized unavailable UI before any account request across Apple, native Android/TV, and KMP. Adult-content coverage verifies explicit age confirmation, matching six-digit PINs, five-attempt/five-minute lockout, local-only state, Worker cache/query partitioning, client-side fail-closed filtering for every entity-related title/credit surface, and companion filtering. Advanced Discover coverage verifies the same fail-closed capability model, `/v1` basic fallback routing, capability-gated Profile provider-region loading, complete type-safe Movie/TV query forwarding, calendar-date rejection, regional provider configuration, canonical fixture decoding, deterministic provider/monetization encoding, context-preserving draft/apply behavior, runtime-bound normalization, and stale request/provider clearing. Worker coverage also includes scheduled Movie/TV/Person change polling, bounded page cursors, 14-day backlog/UTC rollover, invalid cursor and verified changing-page-count recovery, partial upstream failure, replay-safe monotonic D1 revisions, D1-safe 100-item page chunking, change-aware Cache API misses, and cache bypass when a bound D1 revision read fails. The most recent Apple artwork run passed unsigned iOS, Catalyst, tvOS, native macOS, and watchOS builds without Asset Catalog Compiler document warnings. The built iOS bundle contains `UILaunchScreen → UIColorName = LaunchBackground`; tvOS compiles layered small/large icons plus standard/wide Top Shelf artwork; watchOS uses its own complete icon set. visionOS remains subject to the host limitation below.

The native `SmartMovieNativeMac` executable also remained alive for a five-second smoke window, so the earlier exit-code 132 launch crash was not reproduced. Signed-device CloudKit validation is still required before release. Earlier visionOS source compile/link validation passed, but the current Intel host cannot expose a visionOS destination or compile its catalog because no visionOS Simulator runtime is installed. The visionOS target now selects its own two-layer 1024×1024 `VisionAppIcon` stack. CI on Apple silicon captures the build log and rejects Asset Catalog Compiler or missing icon/Top Shelf/launch-screen warnings, making it the authoritative visionOS source-and-asset gate.
